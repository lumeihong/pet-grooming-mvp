...
export default function PayDeposit({ navigation, route }) {
  ...
  const deposit = 10; // 定金 S$

  const pay = async () => {
    setPaying(true);
    // 演示：直接模拟支付成功。真实版在此调用 PayNow/Stripe 网关
  ...
  return (
    <View style={styles.wrap}>
      ...
      <View style={styles.row}>
        <Pill active={method === 'paynow'} onPress={() => setMethod('paynow')}>PayNow（优先）</Pill>
        <Pill active={method === 'card'} onPress={() => setMethod('card')}>信用卡</Pill>
      </View>

      <BigButton label={paying ? '支付中…' : `确认支付 S$ ${deposit}`} onPress={pay} disabled={paying} />
