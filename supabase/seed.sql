...
-- 1) 测试登录账号（auth.users）
-- 注意三处必填，否则登录失败：
--   phone_confirmed_at  —— 未确认会被拒（phone not confirmed）
--   aud / role          —— GoTrue 按 aud='authenticated' 过滤，NULL 报 Invalid login credentials
--   instance_id         —— 固定 '00000000-0000-0000-0000-000000000000'
insert into auth.users (instance_id, id, aud, role, phone, phone_confirmed_at, encrypted_password, raw_app_meta_data, raw_user_meta_data, created_at, updated_at)
values
  ('00000000-0000-0000-0000-000000000000', '11111111-1111-1111-1111-111111111101', 'authenticated', 'authenticated', '+6598000001', now(), crypt('test1234', gen_salt('bf')),
   '{"provider":"phone","providers":["phone"]}', '{"role":"groomer","name":"Asha"}', now(), now()),
  ('00000000-0000-0000-0000-000000000000', '11111111-1111-1111-1111-111111111102', 'authenticated', 'authenticated', '+6598000002', now(), crypt('test1234', gen_salt('bf')),
   '{"provider":"phone","providers":["phone"]}', '{"role":"groomer","name":"Wei Jie"}', now(), now()),
  ('00000000-0000-0000-0000-000000000000', '11111111-1111-1111-1111-111111111103', 'authenticated', 'authenticated', '+6598000003', now(), crypt('test1234', gen_salt('bf')),
   '{"provider":"phone","providers":["phone"]}', '{"role":"groomer","name":"Mei Ling"}', now(), now()),
  ('00000000-0000-0000-0000-000000000000', '11111111-1111-1111-1111-111111111201', 'authenticated', 'authenticated', '+6598000010', now(), crypt('test1234', gen_salt('bf')),
   '{"provider":"phone","providers":["phone"]}', '{"role":"client","name":"Test Owner"}', now(), now())
on conflict (id) do nothing;

-- 兜底：若之前已跑过旧版 seed（账号已存在但缺 aud/role/确认手机），这里补齐
update auth.users
